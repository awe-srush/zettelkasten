...
wordcount = {
  Str = function(el)
    -- we don't count a word if it's entirely punctuation or a zettel link:
    if el.text:match("%P")
      and not el.text:match('^%[%[.*%]%]$')
      and not el.text:match('^%[%[%[.*%]%]%]$') then
        words = words + 1
    end
...
function Pandoc(el)
    ...
    pandoc.walk_block(
      pandoc.walk_block(pandoc.Div(el.blocks), rmLink),
      wordcount
    )
